################################################################
# readPdfTokenizeAndStore.R
#
# Reads in PDFs, Toekizes the text and Stores as a .csv
################################################################

#' @export
readPdfTokenizeAndStore <- function(pathToPDFs, pathForOutput){

# # # #### FOR TESTING #########
#    pathToPDFs <- "PDFs/postChavez/"
#    pathForOutput <- "tokenizedText/"
# # # #### FOR TESTING

# Exract the pdfSetNames from the fielpaths

setName <- str_remove(string = pathToPDFs, pattern = "PDFs/")
setName <- str_remove(string = setName, pattern = "/")
setNameForWrite <- paste("tokenized",toupper(substring(setName, 1, 1)), substring(setName, 2), sep = "")




### TOKENIZE###


# Extract and tokenize the text from each of the preChavez PDFs
# # Combine the tokenized data into one dataframe
setPdfTokensRaw <- extractFromPdfAndTokenizeSet(filePath = pathToPDFs, pdfSetName = setName)



write_csv(x = setPdfTokensRaw, path = paste(pathForOutput,"/",setNameForWrite,".csv", sep="") )


}
